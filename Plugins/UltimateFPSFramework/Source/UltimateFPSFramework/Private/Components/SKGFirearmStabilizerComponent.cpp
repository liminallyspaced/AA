...
bool USKGFirearmStabilizerComponent::FindStabilizingSurface()
{
	...
	if (IsValid(Firearm) && Firearm->GetClass()->ImplementsInterface(USKGFirearmPartsInterface::StaticClass()))
	{// FUNCTION REMOVED
		ActorsIgnored.Append(ISKGFirearmPartsInterface::Execute_GetCachedParts(Firearm));
	}
...
void USKGFirearmStabilizerComponent::StabilizeFirearm()
{
	if (IsValid(Firearm) && IsValid(CharacterComponent) && IsValid(CharacterComponent->GetAnimationInstance()))
	{
		...
		Firearm->SetSwayMultiplier(SwayMultiplier);
		bIsStabilized = true;
...
void USKGFirearmStabilizerComponent::SetSwayMultiplier(float NewMultiplier)
{
	SwayMultiplier = FMath::Clamp(NewMultiplier, 0.0f, 100.0f);
}
