...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnHeldActorChanged, AActor*, OldHeldActor, AActor*, NewHeldActor);
...
protected:
	...
	// Set to true if you have a dedicated camera bone the camera is attached to
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SKGFPSFramework | Default")
	...
	void Server_SetFirearmPose(ESKGFirearmPose NewFirearmPose);
...
protected:
	...
	UFUNCTION(Server, Unreliable, WithValidation)
	void Server_SetUseLeftHandTwoBoneIK(bool bUse);
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "SKGFPSFramework | Leaning")
	ESKGLeaning GetCurrentLeanDirection();
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "SKGFPSFramework | Procedural")
	void SetUseLeftHandTwoBoneIK(bool bUse);
	UFUNCTION(BlueprintPure, Category = "SKGFPSFramework | Procedural")
	bool GetUseLeftHandTwoBoneIK() const { return bUseLeftHandTwoBoneIK; }
	UFUNCTION(BlueprintPure, Category = "SKGFPSFramework | Procedural")
	EAxis::Type GetRightHandAxis() const { return RightHandAxis; }

	UFUNCTION(BlueprintCallable, Category = "SKGFPSFramework | HeldActor")
	void UpdateDefaultCameraFOV(const float FOV, const bool bChangeCameraToNewFOV = false);
	...
	*/
	...
	UPROPERTY(BlueprintAssignable, Category = "SKGFPSFramework | AimingActor")
	FOnHeldActorChanged OnHeldActorChanged;
	UPROPERTY(BlueprintAssignable, Category = "SKGFPSFramework | AimingActor")
	FOnPoseFinished OnHeldActorPoseFinished;
