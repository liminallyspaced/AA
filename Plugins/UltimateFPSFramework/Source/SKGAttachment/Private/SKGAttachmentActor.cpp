...
void ASKGAttachmentActor::PostInitializeComponents()
{
...
void ASKGAttachmentActor::MarkComponentsAsPendingKill()
{
	...
	{
		OwningAttachmentComponent->GetAttachmentManager()->HandleCachingAttachmentComponents();
	}
}
...
void ASKGAttachmentActor::OnRep_AttachmentReplication()
{
	...
	Execute_SetRelativeToOffset(this, GetAttachParentActor());
	ApplyAttachmentAttachOffset();
}
...
void ASKGAttachmentActor::MoveForwardPosition_Implementation(uint8 Amount)
{
	if (USKGAttachmentComponent* AttachmentComponent = GetOwningAttachmentComponent_Implementation())
	{
		const int32 CurrentIndex = AttachmentComponent->GetAttachmentCurrentSnapPointIndex();
		const TArray<float> SnapPoints = AttachmentComponent->GetSnapPoints();
		if (SnapPoints.Num())
		{
			uint8 NewIndex = static_cast<uint8>(CurrentIndex);
			if (NewIndex + Amount >= SnapPoints.Num())
			{
				NewIndex = SnapPoints.Num() - 1;
			}
			else
			{
				NewIndex += Amount;
			...
			CurrentOffset = SnapPoints[NewIndex];
			SetActorRelativeLocation(FVector(AttachmentAttachOffset.GetLocation().X, CurrentOffset + AttachmentAttachOffset.GetLocation().Y, AttachmentAttachOffset.GetLocation().Z));
		}
	}
}

void ASKGAttachmentActor::MoveBackwardPosition_Implementation(uint8 Amount)
{
	if (USKGAttachmentComponent* AttachmentComponent = GetOwningAttachmentComponent_Implementation())
	{
		const int32 CurrentIndex = AttachmentComponent->GetAttachmentCurrentSnapPointIndex();
...
void ASKGAttachmentActor::SetRelativeToOffset_Implementation(AActor* RelativeTo)
{
	if (IsValid(RelativeTo))
	{
		if (USKGAttachmentComponent* Component = Execute_GetOwningAttachmentComponent(this))
		{
			if (!Component->AttachToParentActor())
			{
				return;
			}
		}
		AttachmentAttachOffset = UKismetMathLibrary::MakeRelativeTransform(GetActorTransform(), RelativeTo->GetActorTransform());
		SetActorRelativeLocation(FVector(AttachmentAttachOffset.GetLocation().X, CurrentOffset + AttachmentAttachOffset.GetLocation().Y, AttachmentAttachOffset.GetLocation().Z));
	}
}
