...
USKGAttachmentComponent::USKGAttachmentComponent()
{
	...
	bAttachToParentActor = false;
	bWeldAttachment = true;
	AttachParentActorSocketName = NAME_None;
...
{
	Super::BeginPlay();
	AttachmentTransformReference->SetRelativeTransform(FTransform());
...
#if WITH_EDITOR
void USKGAttachmentComponent::SetPreviewPositions()
{
	if (PreviewSkeletal && PreviewStatic)
	{
		PreviewStatic->AttachToComponent(this, FAttachmentTransformRules::SnapToTargetIncludingScale);
	}
}
...
void USKGAttachmentComponent::PostEditComponentMove(bool bFinished)
{
	Super::PostEditComponentMove(bFinished);
	SetPreviewPositions();
}

void USKGAttachmentComponent::PostEditUndo()
{
	Super::PostEditUndo();
	SetPreviewPositions();
}
...
AActor* USKGAttachmentComponent::AddAttachment(TSubclassOf<AActor> AttachmentClass, bool bDestroyCurrentAttachment)
{
	...
	else
	{
		if (AttachmentClass)
		{
			...
			AActor* SpawnedAttachment = GetWorld()->SpawnActor<AActor>(AttachmentClass, SpawnTransform, SpawnParams);
...
void USKGAttachmentComponent::AddNewAttachment(AActor* INAttachment, bool bDestroyCurrentAttachment)
{
	if (HasAuthority() && IsValid(INAttachment) && INAttachment->GetClass()->ImplementsInterface(USKGAttachmentInterface::StaticClass()))
	{
		...
		TransformRules.bWeldSimulatedBodies = bWeldAttachment;
		if (bAttachToParentActor && GetAttachmentManager() && GetAttachmentManager()->GetOwner())
		{
			INAttachment->AttachToActor(GetAttachmentManager()->GetOwner(), TransformRules, AttachParentActorSocketName);
			ISKGAttachmentInterface::Execute_SetRelativeToOffset(INAttachment, GetAttachmentManager()->GetOwner());
		}
		else
		{
			INAttachment->AttachToComponent(this, TransformRules);	
		}
