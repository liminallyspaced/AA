...
protected:
	...
	// Actor is checked for overlap with attachment dragging system (similar to ground branch to hide/unhide overlapped attachments)
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "SKGAttachment | Default")
	bool bBeCheckedForOverlap;
	// The offset that can be applied when the attachment is attached
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SKGAttachment | Default")
...
public:
	...
	virtual void MoveForwardPosition_Implementation(uint8 Amount) override;
	virtual void MoveBackwardPosition_Implementation(uint8 Amount) override;
	virtual void SetRelativeToOffset_Implementation(AActor* RelativeTo) override;
