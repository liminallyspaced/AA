...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnElevationChanged, FSKGTurretClickEvent, Clicks);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnWindageChanged, FSKGTurretClickEvent, Clicks);
...
protected:
	...
	FSKGTurretAdjustmentSettings TurretAdjustmentSettings;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "SKGFPSFramework | SceneComponent")
	FSKGTurretClickEvent PointOfImpactUp(bool bUp, uint8 Clicks = 1);
	UFUNCTION(BlueprintCallable, Category = "SKGFPSFramework | SceneComponent")
	FSKGTurretClickEvent PointOfImpactLeft(bool bLeft, uint8 Clicks = 1);
	...
	UFUNCTION(BlueprintCallable, Category = "SKGFPSFramework | SceneComponent")
	void SetTurretAdjustment(const FSKGTurretAdjustmentSettings& TurretAdjustment);
};
