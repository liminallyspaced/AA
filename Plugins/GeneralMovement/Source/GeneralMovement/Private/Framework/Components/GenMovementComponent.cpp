...
bool UGenMovementComponent::UpdateFloor(FFloorParams& Floor, float TraceLength)
{
  ...
  if (ShapeHit.bStartPenetrating)
  {
    ...
    if (!bNoAdjustment)
    {
      // The pawn's position was adjusted. Move back as far as possible so we don't lose contact with the ground.
      const FVector ResolvePenetrationAdjustment = UpdatedComponent->GetComponentLocation() - CurrentLocation;
      MoveUpdatedComponent(
        -ResolvePenetrationAdjustment, UpdatedComponent->GetComponentRotation(), true, nullptr, ETeleportType::TeleportPhysics
      );
      const FVector ActualAdjustment = UpdatedComponent->GetComponentLocation() - CurrentLocation;
      FLog(
        Verbose,
